...
export default function CoursePlayer() {
  ...
  async function loadCourse() {
    ...
    // If a lessonId is in the URL, skip auto-select
    if (!params.lessonId) {
  ...
  async function navigateToModule(mod: ModuleWithLessons) {
    // If module has no description and no intro video, skip intro and go straight to first lesson
  ...
  function navigateToSection(section: Section) { setActiveSection(section) }

  function nextSection() {
    if (!activeSection || !lessonSections.length) return
    const idx = lessonSections.findIndex(s => s.id === activeSection.id)
    if (idx < lessonSections.length - 1) setActiveSection(lessonSections[idx + 1])
    else nextLesson()
  }
    ...
    <div className={`min-h-screen flex flex-col ${bg}`}>
      ...
      <nav className={`border-b ${borderCol} px-6 py-3 flex items-center justify-between sticky top-0 ${dark ? 'bg-[#0a0a0a]/95' : 'bg-white/95'} backdrop-blur z-50 shrink-0`}>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setDark(d => !d)} className={`${mutedText} hover:text-current transition-colors ml-2`} title="Toggle theme">
            {dark ? '☀️' : '🌙'}
          </button>
          ...
          {course && (
            <>
              <span className={dark ? 'text-[#333]' : 'text-[#ddd]'}>/</span>
              <span className={`${mutedText} text-sm truncate max-w-[200px]`}>{course.title}</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-3">
          <span className={`${mutedText} text-xs`}>{completedCount}/{totalCount} lessons</span>
          <div className={`w-24 h-1 ${dark ? 'bg-[#1a1a1a]' : 'bg-[#eee]'} rounded-full overflow-hidden`}>
            <div className={`h-full ${dark ? 'bg-white' : 'bg-black'} rounded-full transition-all`} style={{ width: overallProgress + '%' }} />
          </div>
        </div>
      </nav>
            ...
            {modules.map((mod, mIdx) => {
              ...
              return (
                <div key={mod.id} className="mb-6">
                  {/* Module header — clickable if has intro, otherwise just a label */}
                  <button
                    ...
                    className={`w-full text-left px-2 py-1.5 rounded-lg mb-2 transition-colors group flex items-center justify-between gap-2 ${
                      modActive
                        ? dark ? 'bg-[#1a1a1a] text-white' : 'bg-[#f0f0f0] text-black'
                        : dark ? 'text-[#555] hover:text-[#888]' : 'text-[#aaa] hover:text-[#666]'
                    }`}
                  >
                    ...
                    {hasIntro && (
                      <span className={`text-xs opacity-0 group-hover:opacity-100 transition-opacity ${dark ? 'text-[#444]' : 'text-[#ccc]'}`}>
                        intro ›
                      </span>
                    )}
                  </button>
                        ...
                        <div key={lesson.id}>
                          <button
                            ...
                            className={`w-full text-left px-3 py-2.5 rounded-lg flex items-start gap-3 transition-colors ${
                              isActive
                                ? dark ? 'bg-[#1a1a1a] text-white' : 'bg-[#f0f0f0] text-black'
                                : dark ? 'text-[#888] hover:text-[#ccc] hover:bg-[#111]' : 'text-[#666] hover:text-black hover:bg-[#f5f5f5]'
                            }`}
                          >
                            ...
                            return (
                              <button key={sec.id} onClick={() => navigateToSection(sec)}
                                className={`w-full text-left pl-9 pr-3 py-2 flex items-center gap-2 text-xs transition-colors rounded-lg ${
                                  secActive
                                    ? dark ? 'text-white bg-[#222]' : 'text-black bg-[#ebebeb]'
                                    : dark ? 'text-[#666] hover:text-[#aaa] hover:bg-[#111]' : 'text-[#999] hover:text-black hover:bg-[#f5f5f5]'
                                }`}>
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {activeModule && !activeLesson ? (
            ...
            <div className="max-w-3xl mx-auto px-8 py-10">
              {/* Section header */}
              <div className="mb-6">
                <p className={`${mutedText} text-xs uppercase tracking-widest mb-1`}>
                  {activeLesson.title}
              ...
              <div className={`mt-10 flex items-center justify-between border-t ${borderCol} pt-6`}>
                ...
                <div className="flex items-center gap-3">
                  {activeSection && lessonSections.findIndex(s => s.id === activeSection.id) === lessonSections.length - 1 && (
                    ...
                    >
                      {lessonCompleted || activeLesson.progress?.completed ? '✓ Completed' : completing ? 'Saving…' : 'Mark lesson complete'}
                    </button>
                  )}
...
// ─── Module Intro Screen ──────────────────────────────────────────────────────
function ModuleIntroScreen({ mod, dark, onStartModule }: {
  mod: ModuleWithLessons; dark: boolean; onStartModule: () => void
}) {
    ...
    <div className="max-w-2xl mx-auto px-8 py-16">
      ...
      {mod.description && (
        <div className={`border-l-2 ${dark ? 'border-[#2a2a2a]' : 'border-[#e0e0e0]'} pl-5 mb-10`}>
          <p className={`${dark ? 'text-[#aaa]' : 'text-[#555]'} leading-relaxed text-base whitespace-pre-wrap`}>
            {mod.description}
          </p>
        </div>
      )}
    ...
    <div className="space-y-6">
      {questions.map((q: any, qi: number) => (
        <div key={qi} className="border border-[#1a1a1a] rounded-xl p-5 bg-[#0f0f0f]">
          ...
          {q.type === 'fill_blank' && (
            <input value={answers[qi] || ''} onChange={e => !submitted && setAnswers(a => ({ ...a, [qi]: e.target.value }))}
      ...
      {!submitted ? (
        <button onClick={() => setSubmitted(true)} className="px-6 py-2.5 bg-white text-black rounded-lg text-sm font-medium hover:bg-white/90 transition-colors">
          Submit answers
        </button>
      ) : (
        <div className="flex items-center gap-3">
...
function TextContent({ text }: { text: string }) {
  ...
  if (isHtml) {
    return <div className="text-[#ccc] leading-relaxed" style={{ lineHeight: 1.8 }} dangerouslySetInnerHTML={{ __html: text }} />
  }
  return (
