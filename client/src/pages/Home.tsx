...
export default function Home() {
  ...
  useEffect(() => {
    supabase
      .from('posts')
      .select('slug, title, excerpt, category, thumbnail_url, published_at')
  ...
  return (
    <div>
      <SEO
        title="FinVerse — Institutional Market Structure Journal"
        description="Hybrid institutional market structure for disciplined independent traders. Smart Money Concepts, classical technical rhythm, and risk architecture."
        canonical="/"
        noSuffix
      ...
      <section className="relative w-full h-[85vh] min-h-[600px] overflow-hidden">
        ...
        <div className="relative h-full flex flex-col justify-end pb-20 md:pb-24 px-5 mx-auto" style={{ maxWidth: "1200px" }}>
          <div className="max-w-2xl">
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#9EA7B3] mb-5">Market Structure Journal</p>
            <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold leading-[1.1] tracking-tight text-white mb-6">
              Structural Precision.<br />Disciplined Execution.
            </h1>
      ...
      <section className="py-24 md:py-32" style={{ backgroundColor: "#111318" }}>
        <div className="px-5 mx-auto" style={{ maxWidth: "1200px" }}>
          <div style={{ maxWidth: "720px" }}>
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#9EA7B3] mb-6">The Hybrid Thesis</p>
            <h2 className="font-serif text-3xl md:text-4xl font-bold leading-tight text-[#F4F4F2] mb-8">Three Disciplines. One Framework.</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-10 md:gap-14 mt-14">
            {[
              { title: "Liquidity Mechanics", body: "Smart Money Concepts explain where institutional order flow concentrates — liquidity pools, order blocks, and fair value gaps define the zones where price is engineered to move." },
              { title: "Classical Rhythm",    body: "Classical structure defines the rhythm of price — higher highs, lower lows, breaks of structure, and changes of character provide the temporal framework for directional bias." },
              { title: "Risk Architecture",   body: "Risk architecture governs capital survival — position sizing, session filters, and graduated allocation ensure that no single trade or sequence of trades compromises the account." },
            ].map(({ title, body }) => (
              <div key={title} style={{ borderLeft: "1px solid #3E5C76", paddingLeft: "1.5rem" }}>
                <h4 className="font-serif text-lg font-bold text-[#F4F4F2] mb-3">{title}</h4>
                <p className="text-sm text-[#9EA7B3]" style={{ lineHeight: "1.7" }}>{body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 md:py-32" style={{ backgroundColor: "#111318" }}>
        <div className="px-5 mx-auto" style={{ maxWidth: "1200px" }}>
          <div className="flex items-end justify-between mb-14">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#9EA7B3] mb-4">Latest Analysis</p>
              <h2 className="font-serif text-3xl md:text-4xl font-bold text-[#F4F4F2]">Featured Articles</h2>
            </div>
            <Link href="/blog" className="hidden md:inline-flex items-center gap-2 text-sm text-[#3E5C76] hover:text-[#9EA7B3] transition-colors">
              View All <ArrowRight size={14} />
            </Link>
          </div>

          {featured.length === 0 ? (
            <p className="text-[#9EA7B3] text-sm">No articles published yet.</p>
          ) : (
              ...
              {featured.map(post => (
                <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                  <div className="aspect-[16/10] overflow-hidden mb-5" style={{ backgroundColor: "#1a1d24" }}>
                    {post.thumbnail_url
                      ? <img src={post.thumbnail_url} alt={post.title} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-300" loading="lazy" />
                      : <div className="w-full h-full flex items-center justify-center"><span className="text-[#3E5C76] text-xs uppercase tracking-widest">{post.category}</span></div>
                    }
                  </div>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-xs font-medium uppercase tracking-[0.08em] text-[#9EA7B3]">{formatDate(post.published_at)}</span>
                  </div>
                  <h4 className="font-serif text-xl font-bold text-[#F4F4F2] group-hover:text-[#3E5C76] transition-colors leading-snug mb-2">{post.title}</h4>
                  <p className="text-sm text-[#9EA7B3] leading-relaxed line-clamp-2">{post.excerpt}</p>
                </Link>
              ))}
            </div>
          )}
          ...
          <div className="md:hidden mt-10">
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-[#3E5C76]">
        ...
        <div className="px-5 mx-auto" style={{ maxWidth: "1200px" }}>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-10">
            <div style={{ maxWidth: "480px" }}>
              <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#9EA7B3] mb-4">Weekly Structure Watch</p>
              <h2 className="font-serif text-2xl md:text-3xl font-bold text-[#F4F4F2] mb-3">Key Levels. No Noise.</h2>
            ...
            <form onSubmit={handleSubscribe} className="flex w-full md:w-auto">
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                ...
                style={{ backgroundColor: "rgba(158,167,179,0.08)", border: "1px solid rgba(158,167,179,0.2)", borderRight: "none" }} />
              <button type="submit"
