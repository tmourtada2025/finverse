...
type Section = 'profile' | 'catalogue' | 'courses'
...
function NotificationBell({ userId, t }: { userId: string; t: any }) {
  ...
  async function markAllRead() {
    ...
    await supabase.from('notification_reads').upsert(unread.map(n => ({ notification_id: n.id, user_id: userId })))
    setReadIds(new Set(notifications.map(n => n.id)))
  }

  const unreadCount = notifications.filter(n => !readIds.has(n.id)).length
  const typeIcon: Record<string, string> = { announcement: '📢', new_course: '🎓', event: '📅', offer: '🎁' }
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span style={{ position: 'absolute', top: 0, right: 0, background: '#ef4444', color: '#fff', fontSize: '0.6rem', fontWeight: 700, borderRadius: '999px', minWidth: '16px', height: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px', lineHeight: 1 }}>
            {unreadCount > 9 ? '9+' : unreadCount}
          </span>
        )}
      </button>
      {open && (
        <div style={{ position: 'absolute', top: '100%', right: 0, zIndex: 999, width: '320px', backgroundColor: t.bg, border: `1px solid ${t.border}`, borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.15)', overflow: 'hidden', marginTop: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: `1px solid ${t.border}` }}>
            <span style={{ fontSize: '0.875rem', fontWeight: 600, color: t.text }}>Notifications</span>
            {unreadCount > 0 && <button onClick={markAllRead} style={{ fontSize: '0.72rem', color: t.muted, background: 'none', border: 'none', cursor: 'pointer' }}>Mark all read</button>}
          </div>
          <div style={{ maxHeight: '380px', overflowY: 'auto' }}>
            {notifications.length === 0 ? (
              <div style={{ padding: '2rem', textAlign: 'center', color: t.muted, fontSize: '0.855rem' }}>No notifications yet</div>
            ) : notifications.map(n => (
              <div key={n.id} onClick={() => markRead(n.id)} style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, backgroundColor: readIds.has(n.id) ? 'transparent' : t.surface, cursor: 'default' }}>
...
export default function Dashboard() {
  ...
  const t = dark ? {
    ...
    cardBorder: '#1e1e24', green: '#10b981', blue: '#3b82f6', amber: '#f59e0b', indigo: '#6366f1',
  } : {
  ...
  const sectionTitles: Record<Section, string> = { profile: 'Profile', catalogue: 'Course Catalogue', courses: 'My Courses' }
        ...
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${t.sidebarBorder}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: '0.82rem', fontWeight: 500, color: t.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile?.full_name || user?.email?.split('@')[0]}</p>
              <p style={{ fontSize: '0.7rem', color: t.muted, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile?.email || user?.email}</p>
            </div>
          </div>
        </div>
        ...
        <nav style={{ flex: 1, padding: '10px', overflowY: 'auto' }}>
            ...
            return (
              <button key={id} onClick={() => setSection(id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', padding: '9px 12px', borderRadius: '8px', border: 'none', cursor: 'pointer', textAlign: 'left' as const, marginBottom: '1px', transition: 'all 0.12s', backgroundColor: active ? t.activeNavBg : 'transparent', color: active ? t.activeNavText : t.navText, fontWeight: active ? 500 : 400, fontSize: '0.855rem' }}>
                ...
                {id === 'courses' && enrollments.length > 0 && (
                  <span style={{ marginLeft: 'auto', fontSize: '0.65rem', backgroundColor: active ? (dark ? '#2a2a38' : '#e0e0ea') : t.dim, color: t.muted, borderRadius: '10px', padding: '1px 6px' }}>{enrollments.length}</span>
                )}
              </button>
            )
          })}
          {isAdmin && (
            <>
              ...
              <Link href="/admin">
                <button style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', padding: '9px 12px', borderRadius: '8px', border: 'none', cursor: 'pointer', textAlign: 'left' as const, backgroundColor: 'transparent', color: t.amber, fontWeight: 400, fontSize: '0.855rem' }}>
        ...
        <div style={{ padding: '28px 32px' }}>
          {section === 'courses' && (
            enrollments.length === 0 ? (
              <div style={{ border: `1px solid ${t.border}`, borderRadius: '12px', padding: '52px 24px', textAlign: 'center' as const }}>
                ...
                <button onClick={() => setSection('catalogue')} style={{ backgroundColor: t.accent, color: t.accentText, border: 'none', borderRadius: '8px', padding: '10px 20px', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer' }}>Browse catalogue →</button>
              </div>
            ) : (
            ...
            catalogue.length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(256px, 1fr))', gap: '14px' }}>
                {catalogue.map(course => {
                  ...
                  return (
                    <div key={course.id} style={{ border: `1px solid ${t.cardBorder}`, borderRadius: '12px', padding: '20px', backgroundColor: t.surface, display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
                      <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
                        ...
                        {enrolled
                          ? <Link href={`/learn/${course.id}`}><span style={{ fontSize: '0.78rem', padding: '6px 14px', borderRadius: '7px', border: `1px solid ${t.border}`, cursor: 'pointer', color: t.text, fontWeight: 500 }}>Open →</span></Link>
...
function CourseCard({ enrollment, t }: { enrollment: EnrollmentWithCourse; t: any }) {
  ...
  async function determineState() {
    if (enrollment.completed_at) { setCardState('review'); return }
      ...
      <div style={{ border: `1px solid ${t.border}`, borderRadius: '12px', overflow: 'hidden', backgroundColor: t.surface }}>
        ...
        <div style={{ padding: '22px', display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div><label style={lbl}>First name</label><input value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="First name" style={input} /></div>
            <div><label style={lbl}>Last name</label><input value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Last name" style={input} /></div>
          </div>
          <div>
